...
# '''
# 	list_start = '''
# <ul style="margin-left:30px;margin-top:-40px; list-style-type:none">\n
# '''
# 	list_end = '</ul>\n'
# 
# 	for pub in pubs:
# 		# title
# 		pub['url_base'] = url_base
# 		if pub['label'] != 'x' and 'paper' in pub['files']:
# 			text = '<a href="%(url_base)s%(label)s_paper.pdf" style="font-weight:bold;">%(title)s</a>' % pub
# 		elif 'url' in pub:
# 			text = '<a href="%(url)s" style="font-weight:bold;">%(title)s</a>' % pub
# 		else:
# 			text = '<div style="float:left;font-weight:500;">%(title)s</div>' % pub
# 		# author
# 		authors = pub['author']
# 		author_list = pub['author'].split(' and ')
# 		if len(author_list) > 2:
...
# 		# other text
# 		if 'booktitle' in pub:
# 			text += ', %s' % pub['booktitle']
# 		if 'year' in pub:
# 			text += ' %s' % pub['year']
# 		text += '.'
# 		# links
# 		if pub['label'] != 'x':
# 			out = open(write_dir + pub['label'] + '.bib', 'w')
# 			bib_comb = []
# 			for line in pub['bib']:
# 				print >> out, line[:-1]
# 				bib_comb.append(line[:-1])
# 			pub['bib_comb'] = '\n'.join(bib_comb)
# 			out.close()
# 			text += '&nbsp;&nbsp;&nbsp;&nbsp;'
# 			for ending in pub['files']:
# 				text += '''<a href="%s%s_%s">[%s]</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;''' % (pub['url_base'], pub['label'], pub['files'][ending], ending)
# 			text += '''<a href="javascript:;" onClick="
# if (document.getElementById('bib%(num)s').style.display == 'block') {
# 	document.getElementById('bib%(num)s').style.display='none';
# } else {
# 	document.getElementById('bib%(num)s').style.display='block';
...
# 		</code>
# 	</blockquote>
# &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="%(url_base)s%(label)s.bib">bib as a file</a>
# </div>\n
# ''' % pub
# 		pub['html'] = '<li style="margin:0px 0px 10px 0px;">\n%s\n</li>\n' % text
# 	by_year = {}
# 	for pub in pubs:
# 		if 'year' in pub:
# 			if pub['year'] not in by_year:
# 				by_year[pub['year']] = []
# 			by_year[pub['year']].append(pub)
# 	years = by_year.keys()
# 	years.sort(reverse=True)
# 
# 	ans += '<p><div style="width:100%;margin-top:-20px;padding-left:20px;">\n'
# 	for year in years:
# 		ans += '<div style="width:9%;float:left;">\n'
# 		ans += '<h3><a href="#%s">%s</a></h3>\n' % (year, year)
# 		ans += "</div>\n"
# 	ans += '''</div></p>
# 	<p>
# 	<div style="width:100%;">\n'''
# 	for year in years:
# 		ans += '<p><a name="%s"></a><h3>%s</h3>\n' % (year, year)
# 		ans += list_start
